...
const Cell: React.FC<CellProps> = ({ value, onClick, isSelected }) => {
  ...
  const baseStyle = `w-16 h-16 border ${isSelected ? 'border-red-500 border-4' : 'border-gray-400'} flex items-center justify-center text-xl font-bold cursor-pointer select-none transition-colors duration-300`;
  // 値に応じて背景色を変える
  const colorClasses = generateColorClasses(getNumBlockTypes());
  const colorStyle = value !== null && colorClasses[value] ? colorClasses[value] : 'bg-gray-200';
