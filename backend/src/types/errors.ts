...
// 認証エラー用のヘルパー
export const authError = (c: Context, message = 'Authentication required'): Response => {
  return errorResponse(c, new ScoreError(message, 'AUTH_REQUIRED'), 401);
};

// データベースエラー用のヘルパー
export const dbError = (c: Context, message = 'Database operation failed'): Response => {
  return errorResponse(c, new ScoreError(message, 'DB_ERROR'), 500);
};

// レート制限エラー用のヘルパー
export const rateLimitError = (c: Context, message = 'Rate limit exceeded'): Response => {
